...
- A Hex account that owns (or co-owns) the `butteraugli` package.
...
1. **Decide the version** and confirm `butteraugli` is still pinned where you
   want it ([native/butteraugli_nif/Cargo.toml](native/butteraugli_nif/Cargo.toml) —
   currently a git rev, by design).

2. **Bump the version in both places** (keep them in lockstep):
   - [mix.exs](mix.exs) — `version:`
   - [native/butteraugli_nif/Cargo.toml](native/butteraugli_nif/Cargo.toml) — `version`

   `lib/butteraugli/native.ex` derives the release download URL
...
4. **Verify the build is clean** (mirrors [CI](.github/workflows/ci.yml)):
   ...
   mise exec -- cargo fmt --manifest-path native/butteraugli_nif/Cargo.toml --check
   mise exec -- mix format --check-formatted
   BUTTERAUGLI_BUILD=1 mise exec -- mix compile --warnings-as-errors --force
...
7. **Generate the checksum file** (downloads and hashes every artifact):
   ...
   ```bash
   mise exec -- mix rustler_precompiled.download Butteraugli.Native --all --print
   ```

   This writes `checksum-Elixir.Butteraugli.Native.exs`. It is matched by the
   `checksum-*.exs` entry in the `:files` of [mix.exs](mix.exs) and **must** be
   present at publish time — without it, precompiled NIF loading fails for
   consumers. (It is a build artifact, not committed to git; it just needs to
   exist locally when you publish.)
...
8. **Publish to Hex:**
   ...
   `native/butteraugli_nif/Cargo.lock`, `lib`, `README.md`, and `CHANGELOG.md`
   are all included — then confirm. Docs are built and pushed automatically
   (`ex_doc`).
